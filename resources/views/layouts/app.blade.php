...
<body class="bg-grey-50 text-grey-900 font-sans">
    <!-- Modern Navbar -->
    <nav class="bg-[#B2C6D5] shadow-md sticky top-0 z-50 backdrop-blur-sm bg-opacity-90">
            ...
            <div class="flex justify-between items-center h-16">
                ...
                <div class="flex items-center space-x-4">
                    ...
                    <button id="mobile-menu-button"
                        class="md:hidden text-grey-500 hover:text-grey-700 focus:outline-none">
                ...
                <div class="hidden md:flex items-center space-x-8">
                        ...
                        <form action="#" method="GET">
                            <div class="relative">
                                <input type="text" name="query" placeholder="Search articles..."
                                    class="w-full pl-4 pr-10 py-2 border border-grey-200 rounded-full focus:outline-none focus:ring-2 focus:ring-blue-400 focus:border-transparent transition-all duration-200 shadow-sm hover:shadow-md">
                                <button type="submit"
                                    class="absolute right-3 top-2 text-grey-400 hover:text-blue-600 transition-colors">
                    ...
                    <div class="flex space-x-6">
                        <a href="{{ route('galleries.index') }}"
                            class="text-grey-600 hover:text-blue-600 transition-colors flex items-center space-x-1 group">
                            <svg xmlns="http://www.w3.org/2000/svg"
                                class="h-5 w-5 text-grey-400 group-hover:text-blue-500 transition-colors"
                    ...
                    @auth
                        ...
                        <div class="relative">
                            <button id="user-dropdown-button" class="flex items-center space-x-1 focus:outline-none group">
                                ...
                                <svg xmlns="http://www.w3.org/2000/svg"
                                    class="h-4 w-4 text-grey-500 group-hover:text-grey-700 transition-colors"
                                    fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                        d="M19 9l-7 7-7-7" />
                                </svg>
                            ...
                            <div id="user-dropdown-menu"
                                class="hidden absolute right-0 mt-2 w-48 bg-white rounded-md shadow-lg py-1 z-50 border border-grey-100">
                                <a href="#"
                                    class="block px-4 py-2 text-sm text-grey-700 hover:bg-grey-100 transition-colors">My
                                    Profile</a>
                                <a href="#"
                                    class="block px-4 py-2 text-sm text-grey-700 hover:bg-grey-100 transition-colors">Settings</a>
                                @if (auth()->user()->is_admin)
                                <a href="{{ route('admin.dashboard') }}"
                                    class="block px-4 py-2 text-sm text-grey-700 hover:bg-grey-100 transition-colors flex items-center">
                                ...
                                <form method="POST" action="{{ route('logout') }}">
                                    @csrf
                                    <button type="submit"
                                        class="block w-full text-left px-4 py-2 text-sm text-grey-700 hover:bg-grey-100 transition-colors">Logout</button>
                                </form>
                            </div>
                        </div>
                    @else
                        <a href="{{ route('login') }}"
                            class="hidden md:inline-flex items-center px-4 py-2 border border-blue-600 text-sm font-medium rounded-md text-white bg-blue-600 hover:bg-blue-700 shadow-sm transition-colors duration-200">
                            Login
                        </a>
                    @endauth
                </div>
        ...
        <div id="mobile-menu" class="hidden md:hidden bg-white border-t border-grey-200">
            <div class="px-2 pt-2 pb-3 space-y-1">
                <form action="#" method="GET" class="px-2">
                    <div class="relative">
                        <input type="text" name="query" placeholder="Search..."
                            class="w-full pl-4 pr-10 py-2 border border-grey-200 rounded-full focus:outline-none focus:ring-2 focus:ring-blue-400 focus:border-transparent">
                        <button type="submit" class="absolute right-3 top-2 text-grey-400 hover:text-blue-600">
                            <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" fill="none"
                                viewBox="0 0 24 24" stroke="currentColor">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                    d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z" />
                ...
                <a href="{{ route('galleries.index') }}"
                    class="block px-3 py-2 rounded-md text-base font-medium text-grey-700 hover:text-blue-600 hover:bg-grey-50 transition-colors">Gallery</a>
                <a href="#"
                    class="block px-3 py-2 rounded-md text-base font-medium text-grey-700 hover:text-blue-600 hover:bg-grey-50 transition-colors">Articles</a>
                ...
                @auth
                    ...
                    <form method="POST" action="{{ route('logout') }}">
                        @csrf
                        <button type="submit"
                            class="block w-full text-left px-3 py-2 rounded-md text-base font-medium text-grey-700 hover:text-blue-600 hover:bg-grey-50 transition-colors">Logout</button>
                    </form>
                @else
                    <a href="{{ route('login') }}"
                        class="block px-3 py-2 rounded-md text-base font-medium text-grey-700 hover:text-blue-600 hover:bg-grey-50 transition-colors">Login</a>
            ...
            <div class="grid grid-cols-1 md:grid-cols-4 gap-8">
              ...
              text-blue-600 font-bold text-sm uppercase tracking-wider shadow-sm">
                    ...
                    <p class="mt-4 text-grey-500 text-sm">
                       Menyediakan layanan dan informasi perawatan kesehatan berkualitas untuk membantu Anda menjalani hidup yang lebih sehat.
                    </p>
                    <div class="mt-4 flex space-x-4">
                        ...
                        <a href="https://www.instagram.com/rohyoonseo/?hl=id" class="text-grey-400 hover:text-blue-500 transition-colors">
                            <svg class="h-6 w-6" fill="currentColor" viewBox="0 0 24 24" aria-hidden="true">
                                <path fill-rule="evenodd"
