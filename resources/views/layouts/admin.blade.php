...
<body class="flex min-h-screen bg-gray-100 text-gray-900">
    ...
    <!-- Sidebar -->
    <!-- Sidebar -->
    <aside class="w-64 bg-white shadow-lg min-h-screen p-4 flex flex-col justify-between">
        <div>
            <h1 class="text-2xl font-bold mb-8 ml-3 mt-3">Admin Panel</h1>
            <nav class="space-y-2">
                <a href="{{ route('admin.dashboard') }}"
                    class="block px-3 py-2 rounded-lg hover:bg-gray-200 {{ request()->routeIs('admin.dashboard') ? 'bg-gray-200' : '' }}">Dashboard</a>
                <a href="{{ route('admin.article.index') }}"
                    class="block px-3 py-2 rounded-lg hover:bg-gray-200 {{ request()->routeIs('admin.article.*') ? 'bg-gray-200' : '' }}">Artikel</a>
                ...
                <form action="{{ route('logout') }}" method="POST"> @csrf <button type="submit"
                        class="w-full text-left block px-3 py-2 rounded-lg hover:bg-gray-200">Logout</button> </form>
            </nav>
        </div>
        ...
        <div class="space-y-3">
            ...
            <a href="{{ route('home') }}"
                class="block px-3 py-2 rounded-lg bg-blue-100 text-blue-700 hover:bg-blue-200 transition mt-1">
                ← Kembali ke Home
            </a>
        </div>
    </aside>
